...
int search_destroy ( char source [], char find [], char replace [] )
 
 {
	...
	void remString ( char text [], int index, int noe, int iter ) ;
	...
	int iter = strlen (source) - ( index + strlen (find) ) ;		//Calculating number of iterations for the remove string function
	remString ( source, index, strlen (find), iter ) ;		//Removing located sub string from main string
