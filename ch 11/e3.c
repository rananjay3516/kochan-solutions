...
int main (void)
 {
	...
	ll_insert (&cos, &zero) ;		//Function call to insert it into linked list
	list = zero.next ;		//Pointer to first printable var **
	while (list)		//Printing list
	 {
	   printf ( "%i ", list->value ) ;
...
//Function to insert an element in linked list - taking element after which to insert as argument
void ll_insert (struct entry *insert, struct entry *listElement)
 {
 	insert->next = listElement->next ;		//Placing insert element before the element succeeding target element 
	listElement->next = insert ;		//Placing insert element after the target element
 }